 ...
 *
 * The name is at full opacity from the first frame and only ever transformed.
 * It is the largest text on the page and therefore the LCP element; fading it
 * in would trade a real load metric for an effect nobody asked for.
 *
          ...
          <p className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-label uppercase">
            {hero.roles.map((role, i) => (
              <span key={role} className="flex items-center gap-3">
                ...
                <span
                  className={i === 0 ? "text-signal" : "text-muted-foreground"}
                >
                  {role}
                </span>
              </span>
